...
- **Arrangement track** — A horizontal timeline lane holding zero or more clip instances, mapped to one instrument/timbre (`presetIndex` — **advisory in V1**: the engine has a single global preset, so two different-timbre tracks can't both sound at the same instant; this is surfaced, not solved — ADR-0007). Has header controls: name, colour, mute, solo (V1); volume, pan (later). Distinct from the audio-input **Track** above.
- **Clip shelf** — The panel beside the Song timeline listing the user's saved compositions (building blocks) as draggable cards; the source from which clip instances are dragged onto tracks (DESIGN-002 / TS-001).
